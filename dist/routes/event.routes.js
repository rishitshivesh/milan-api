...
router.get("/events/:id", async (req, res) => {
  try {
    ...
    const event = await DB.queryWithFilter("milan", `event`, "id", id, TABLE_NAME);
...
router.post("/event/register", isAuthenticated, async (req, res) => {
  try {
    ...
    const event = await DB.queryWithFilter("milan", `event`, "id", eventId, TABLE_NAME);
    if (!event) {
    ...
    const userEvent = await DB.get("milan", `userEvent#${emailId}#${eventId}`, TABLE_NAME);
    if (userEvent) {
      return res.status(400).json(Response(400, "User already registered for this event"));
    }
    if (event.isSrmEvent && !verifyEmail(emailId)) {
      return res.status(400).json(Response(400, "Event is exclusive to SRM Students"));
    }
    // console.log(event);
    const newUserEvent = {
      pk: "milan",
    ...
    return res.status(200).json(Response(200, "User registered for event successfully", event));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
router.post("/event/register/team", isAuthenticated, async (req, res) => {
  try {
    ...
    if (!teamName && !teamId) {
      return res.status(400).json(Response(400, "Either create or Join a Team"));
    }
    const event = await DB.queryWithFilter("milan", `event`, "id", eventId, TABLE_NAME);
    const user = await DB.get("milan", `user#${emailId}`, TABLE_NAME);
    if (!user) {
    ...
    if (teamName) {
      const newTeamId = `MILAN-${event.Id}-TEAM-` + Math.floor(40000 + Math.random() * 9899);
      const newTeam = {
        pk: "milan",
    ...
    return res.status(200).json(Response(200, "User registered for event successfully"));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
...
router.get("/event/users/:eventId", isAuthenticated, async (req, res) => {
  try {
    ...
    const users = await DB.queryBeginsWith("milan", `eventParticipants#${eventId}`, TABLE_NAME);
    var lst = [];
    ...
    return res.status(200).json(Response(200, "Users fetched successfully", lst));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
