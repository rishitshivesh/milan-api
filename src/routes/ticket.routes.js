...
router.post("/ticket/add", isPaymentAuthenticated, async (req, res) => {
  try {
    const { emailId, pID, ticketPrice } = req.body;
    ...
    return res
      .status(200)
      .json(Response(200, "Ticket created successfully", result));
  } catch (error) {
...
router.put("/ticket/update/:id", isAuthorized, async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(Response(200, "Ticket updated successfully", result));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
...
router.get("/ticket/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(Response(200, "Ticket fetched successfully", ticket));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
