...
const { registerSchema } = require("../middlewares/user.middleware");
...
router.post("/admin/user", async (req, res) => {
  try {
    const { emailId, contactNum, regNo, fullName, userType, uID } = req.body;
    const adminDetails = {
      pk: "milan",
      sk: `admin#${emailId}`,
      emailId,
    ...
    return res
      .status(200)
      .json(Response(200, "User added successfully", result));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
...
router.get("/admin/user/:emailId", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(Response(200, "Admin details fetched successfully", users));
  } catch (error) {
    return res.status(500).json(Response(500, "Internal Server Error", error));
  }
});
